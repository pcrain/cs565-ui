var color  = ["#FF0000","#FFFF00","#00FF00","#FFFFFF"];
var status = ["not started","inactive","active","completed"];

var chance = new Chance("test");
...
class Simulator
{
    constructor (groups, weights, nSims, avgCompletionTime, timeInterval, nMaxWords){
        ...
        var avgProgressPerTick = avgCompletionTime*60 /timeInterval;
        ...
        for (var i = 0; i < this.nSims; i++){
            var ind = groupNum[i];
            this.theSims.push(
                new Sim(simNames[i], groups[ind].latencyMean, groups[ind].focusMean, groups[ind].persistenceMean, groups[ind].speedMean, avgProgressPerTick)
            );
...
var group1 = {      //arrived early, average focus, persistence, and speed
    latencyMean : 0,
    focusMean   : 3/5,
    persistenceMean : 3/5,
    speedMean : 0
...
var group2 = {      //arrived early, low focus and persistence, but average speed
    latencyMean : 0,
    focusMean   : 2/5,
    persistenceMean : 1/5,
    speedMean : 0
...
var group3 = {      //arrived early, low focus and persistence, but fast speed
    latencyMean : 0,
    focusMean   : 2/5,
    persistenceMean : 1/5,
    speedMean : 0.50
...
var group4 = {      //arrived within three minutes, average focus but lower persistence and slow speed
    latencyMean : 1/3,
    focusMean   : 3/5,
    persistenceMean : 2/5,
    speedMean : -0.20
...
var group5 = {      //arrived later, good focus but lower persistence and average speed
    latencyMean : 7/10,
    focusMean   : 4/5,
    persistenceMean : 2/5,
    speedMean : 0.00
...
var group6 = {      //arrived later, average focus and lower persistence, but fast speed
    latencyMean : 7/10,
    focusMean   : 4/5,
    persistenceMean : 2/5,
    speedMean : 0.20
...
var nSims             = 15;
var avgCompletionTime = 5; //minutes
var nMaxWords         = 300;

var timeInterval = 3; //seconds

var simulator = new Simulator(groups, weights, nSims, avgCompletionTime, timeInterval, nMaxWords);
simulator.tick();

window.setTimeout(tick(simulator), timeInterval*1000);
